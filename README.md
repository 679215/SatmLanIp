...
1. 双方都安装本模组。
2. 游玩菜单 → **局域网联机**。
3. **房主：** 创建房间 → 选档/模式 → 告知局域网或 VPN IP。
4. **加入方：** 加入 → 输入房主 IP → 连接（改过配置端口时用 `IP:端口`）。
5. 全员准备 → 按大厅提示开始。
...
不在同一局域网时，先用 **VPN / 虚拟局域网** 把双方连到同一虚拟网段，再按上面步骤用 **VPN 分配的 IP** 开房、加入。
...
- [Tailscale](https://github.com/tailscale/tailscale) — 基于 WireGuard 的虚拟组网，跨平台、配置简单；联机时使用 Tailscale 分配给房主的 IP。
- [connecttool-qt](https://github.com/moeleak/connecttool-qt) — 图形化异地组网工具，支持 TUN 虚拟网卡模式；联机时使用组网后显示的虚拟 IP。
