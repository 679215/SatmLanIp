...
public sealed class LanLocalIpTests
{
    ...
    public void LanLocalIp_classifies_physical_adapters()
    {
        ...
        Assert.True(LanLocalIp.IsPhysicalAdvertiseAdapter(NetworkInterfaceType.GigabitEthernet));
        Assert.True(LanLocalIp.IsPhysicalAdvertiseAdapter(NetworkInterfaceType.FastEthernetT));
    ...
    public void LanLocalIp_keeps_overlay_adapters_but_skips_host_only_adapters()
    {
        ...
        Assert.True(LanLocalIp.IsOverlayAdvertiseAdapter("Tailscale", "Tailscale Tunnel"));
        Assert.True(LanLocalIp.IsOverlayAdvertiseAdapter("本地连接 2", "TAP-Windows Adapter V9"));
        Assert.False(LanLocalIp.IsJunkAdvertiseAdapter("WLAN", "Intel Wi-Fi"));
        Assert.False(LanLocalIp.IsJunkAdvertiseAdapter("", ""));
    ...
    public void LanLocalIp_formats_multiple_advertise_rows()
    {
        var rows = new List<LanLocalIp.AdvertiseAddr>
        {
            ...
            new() { Kind = LanLocalIp.KindOther, Ip = "100.64.1.2" },
        };
    ...
    [Fact]
    public void LanLocalIp_IsOwnLanIp_false_for_foreign_address()
    {
        Assert.False(LanLocalIp.IsOwnLanIp("203.0.113.50"));
    }

    [Fact]
    public void LanLocalIp_IsOwnLanIp_true_for_listed_advertise_ip()
    {
        var mine = LanLocalIp.ListIPv4();
        if (mine.Count == 0)
            return;

        Assert.True(LanLocalIp.IsOwnLanIp(mine[0]));
    }
}
