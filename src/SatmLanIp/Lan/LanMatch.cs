...
internal static class LanMatch
{
    ...
    public static void Tick()
    {
        ...
        if (sess != null && !sess.IsHost && !_clientFired)
        {
            ...
            {
                TryStartClient();
                // StartAsClient may park ConnectToCloud mid-Tick.
                FusionCloudBypassPatches.Pump();
            }
        }
    ...
    {
        int current = 0;
        try { current = SaveManager.CurrentSaveSlot; }
        catch { current = 0; }

        SaveFileManager sfm = Find<SaveFileManager>();
        int firstExisting = sfm != null ? FirstExistingSlot(sfm) : -1;
        int slot = LanSession.ResolveSaveSlot(LanMenuFlow.HostSaveSlot, current, firstExisting);
        ...
        if (LanMenuFlow.HostSaveSlot >= 0)
        {
            try { SaveManager.CurrentSaveSlot = slot; }
            catch { /* best-effort */ }
            try { sfm.SelectSaveFile(slot); }
            catch (Exception ex)
            {
                Plugin.LogSrc.LogWarning(
                    "[SatmLanIp] save_reselect fail " + ex.GetType().Name + ": " + ex.Message);
            }
            Plugin.LogSrc.LogInfo("[SatmLanIp] save_select host_picked=" + slot);
            return slot;
        }
        ...
        sfm.CreateNewSave(slot);
        sfm.ConfirmCreateNewSave(false);
        try { SaveManager.CurrentSaveSlot = slot; }
        catch { /* best-effort */ }
        Plugin.LogSrc.LogInfo("[SatmLanIp] save_create slot=" + slot + " story");
        return slot;
    }
