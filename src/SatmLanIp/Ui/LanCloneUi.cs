...
internal static class LanCloneUi
{
    ...
    internal static string FormatLobbyStatus(LanSession s)
    {
        ...
        switch (s.State)
        {
            ...
            case LanState.Connected:
                ...
                // Host keeps advertise so late joiners still see the IP.
                if (s.IsHost)
                {
                    sb.Append('\n');
                    sb.Append(LanLocalIp.FormatAdvertise(Plugin.ListenPort));
    ...
    internal static void SelfCheck()
    {
        ...
        // Host Connected appends advertise IP under the ready line (late joiners).
        if (!FormatLobbyStatus(host).StartsWith("全员已准备  2/2\n", StringComparison.Ordinal))
            throw new InvalidOperationException("SatmLanIp LanCloneUi ready vs seat: " + FormatLobbyStatus(host));
        host.ReadyMask = 1;
        host.LocalReady = true;
        if (!FormatLobbyStatus(host).StartsWith("已准备，等待其他人  1/2\n", StringComparison.Ordinal))
        ...
        if (!FormatLobbyStatus(three).StartsWith("已准备，等待其他人  1/3\n", StringComparison.Ordinal))
            throw new InvalidOperationException("SatmLanIp LanCloneUi 3p ready num: " + FormatLobbyStatus(three));
        three.ReadyMask = 7;
        if (!FormatLobbyStatus(three).StartsWith("全员已准备  3/3\n", StringComparison.Ordinal))
