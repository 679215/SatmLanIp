...
internal static class FusionLanPatches
{
    ...
    private static void StartGamePrefix(StartGameArgs args)
    {
        ...
        try
        {
            ...
            // Match LAN room size so Fusion session capacity follows UI 2/3/6 (not NPC default 3).
            int maxPlayers = s != null ? LanRoom.ClampMax(s.MaxPlayers) : 3;
            args.PlayerCount = new Il2CppSystem.Nullable<int>(maxPlayers);
            Plugin.LogSrc.LogInfo("[SatmLanIp] fusion_PlayerCount=" + maxPlayers);
