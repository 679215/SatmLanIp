...
internal static class FusionCloudBypassPatches
{
    ...
    private static Il2CppStructArray<byte> EnsurePlayerMapping(NetworkRunner runner)
    {
        ...
        try
        {
            ...
            // Host must already know peer UniqueIds before OnConnected, or Fusion throws
            // "no player mapping for <ulong>" and Shutdown Reason=Error (1.5.23 disconnect).
            if (actorId == 1)
            {
                LanSession sess = Plugin.Transport != null ? Plugin.Transport.Session : null;
                int hi = LanFusionStart.HostPremapPeerActorHi(sess != null ? sess.MaxPlayers : 3);
                int n = 0;
                for (int peer = 2; peer <= hi; peer++)
                {
                    Il2CppStructArray<byte> peerUid = MakeUniqueId(peer);
                    sim.RegisterUniqueIdPlayerMapping(peer, peerUid, PlayerRef.FromIndex(peer));
                    n++;
                }
                Plugin.LogSrc.LogInfo(
                    "[SatmLanIp] lan_fusion peer premap actors=2.." + hi
                    + " count=" + n + " maxPlayers=" + hi);
            }
        }
        catch (Exception ex)
        {
    ...
    // PlayerRef.IsRealPlayer is (_index > 0); FromIndex(ResolveLanActorId) fills the empty init window.
    [HarmonyPostfix]
    ...
    {
        if (!LanMatch.AllowFusionStart || !Plugin.VerboseNetworkLog)
            return;
        _nativeRecvCalls++;
        if (_nativeRecvCalls <= 8 || (_nativeRecvCalls % 200) == 0)
    ...
    private static void NativeSendPrefix()
    {
        if (!LanMatch.AllowFusionStart || !Plugin.VerboseNetworkLog)
            return;
        _nativeSendCalls++;
        if (_nativeSendCalls <= 8 || (_nativeSendCalls % 200) == 0)
    ...
    private static void NativeSendPostfix(int __result)
    {
        ...
        if (!Plugin.VerboseNetworkLog)
            return;
        _nativeSendHits++;
        if (_nativeSendHits <= 12 || (_nativeSendHits % 40) == 0)
            Plugin.LogSrc.LogInfo(
    ...
    private static void HybridReceivePrefix()
    {
        if (!LanMatch.AllowFusionStart || !Plugin.VerboseNetworkLog)
            return;
        _hybridRecvCalls++;
        if (_hybridRecvCalls <= 8 || (_hybridRecvCalls % 200) == 0)
    ...
    private static void HybridSendPrefix()
    {
        if (!LanMatch.AllowFusionStart || !Plugin.VerboseNetworkLog)
            return;
        _hybridSendCalls++;
        if (_hybridSendCalls <= 8 || (_hybridSendCalls % 200) == 0)
    ...
    /// <summary>Photon-less Direct ActorNumber: Host=1; client=LocalSlot+1.</summary>
    internal static int ResolveLanActorId()
    {
        LanSession s = Plugin.Transport != null ? Plugin.Transport.Session : null;
        if (s == null)
            return 1;
        return LanFusionStart.ResolveLanActorId(s.IsHost, s.LocalSlot);
    }
